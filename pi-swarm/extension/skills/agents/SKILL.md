...
- `ask_agent({ agent_handle, question, timeout_s? })` — ask a visible askable agent a question and get its answer back. Forks the target read-only when its thread is available; never interrupts or steers it.
- `message_agent({ agent_handle, message, interrupt? })` — send a one-way persistent message to a visible messageable agent, including a parent/session when you know its canonical handle. Returns daemon acceptance (`message_id`/status) only; no recipient answer is included.
...
- Capability is separate from identity:
  - **messageable**: visible agents and sessions can receive `message_agent` by canonical handle.
  - **askable**: visible agents and sessions can be asked when the daemon can fork their thread.
...
`ask_agent({ agent_handle, question, timeout_s? })` consults another visible askable agent without disturbing it:
...
- Parent/root sessions are askable by canonical handle when visible and forkable; they are still not dispatchable or awaitable worker targets.
...
`message_agent({ agent_handle, message, interrupt? })` sends a durable one-way message to another visible messageable agent:
