...
- **Host** (`host/`): process exec + cwd provider, config-file IO, paths, and the typed `BASECAMP_*` env contract with agent-depth helpers.
- **Session lifecycle**: the agent-mode state machine (`analysis` / `planning` / `work` / `copilot`), session start/shutdown, chat compaction, and file-backed session state (`~/.pi/basecamp/core/session-state/`) with fork inheritance.
...
- **Framework UI** (`ui/`): the status footer, title auto-naming, and the interactive mode editor. Feature-specific widgets live with their own domains; only `formatTitle` is consumed externally.
- **Escalate**: the `escalate` tool pauses the session to ask the user for a decision (primary sessions only).
- **Ports**: seams core declares and other domains implement: the tool/skill catalog, model-alias resolution, and the workspace service.
- **Project config** (`project/`): resolves repo → project → `BASECAMP_PROJECT`, the project-config schema, and nested-doc context injection. Registered by the workspace module, whose `session_start` hook it needs; prompt assembly lives in the workspace domain.
- **Workspace defaults**: git detection at `session_start` and worktree operations as thin git wrappers, overridden with config-aware values by the workspace module during registration.
