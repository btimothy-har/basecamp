...
How core touches the runtime it is hosted in: process exec and the cwd provider, config-file IO, paths, and the typed `BASECAMP_*` env contract with agent-depth helpers.
...
The session's chrome: the status footer, title auto-naming, and the interactive mode editor. Feature-specific widgets live with their own domains.
