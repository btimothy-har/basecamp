...
**There is no general intent-alignment rule; approving reviews are the narrow exception.** D6 denies any `gh pr review` invocation that includes `--approve` unless recent human messages explicitly request or authorize submitting an approving review. General PR preparation or update requests, readiness requests, posting comments, and clearing feedback are not approval authorization. Explicit authorization only clears D6: U2 still routes the GitHub write to the interactive user for final publication confirmation.
