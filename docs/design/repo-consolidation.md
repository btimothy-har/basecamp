...
- **No Claude Code launcher implementation.** The layout leaves room for it (`claude/`), but the launcher is [its own design](./claude-code-compatibility.md).
...
4. **Contracts and docs.** Protocol sync test (TS + Python frames vs `swarm/protocol/` schemas and version); rewrite AGENTS.md (repo map, singletons, development/testing sections), core README pattern references, and design-doc links; delete this doc's "DESIGN" status in favor of IMPLEMENTED.
