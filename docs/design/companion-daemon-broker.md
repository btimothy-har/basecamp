...
**Status:** IMPLEMENTED (the analysis channel is now daemon-brokered); the daemon-side broker was later relocated by the swarm→hub rename to `src/basecamp/hub/broker/`. The producer + transport now live in **`pi/core/hub/`** (`thread-reporter.ts` + `report-thread.ts`), registered by `registerCore` — raw-thread reporting is core "connect + report", not a companion job, which **revises §4.1's "companion owns the policy"** (see [hub-core-connector.md](./hub-core-connector.md) §12). · **Scope:** Supersedes the file-based producer/consumer seam and per-turn subprocess analyzer of `repo-rearchitecture.md` §6.4; the Textual UI, pane adapters (herdr/tmux), and Herdr metadata are retained · **Depends on:** the daemon (`async-agents.md`)
