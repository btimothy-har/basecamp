...
> **Revision v4 (2026-07) — attach model, built.** The C1 implementation refined the record model past what §3.4–§3.6 below describe; where they differ, the code is authoritative. Net changes: (1) **the record stores no worktree path and no status.** Agents *attach* to a workstream (`workstream_sessions`: `workstream_id`, `session_id`, `repo`, `worktree_path`, `joined_at`) — each carrying its own repo/worktree — so a workstream is multi-worker and portable across repos, not bound to one worktree. (2) **Liveness is derived, not stored:** a workstream is "live" iff an attached session has an open `episodes` row; there is no `open`/`closed` field, no `set_workstream_status`, and no `/status` route. SessionEnd needs no workstream write — closing the session's episode already makes it idle. The "prune audit" is the derived idle set (`?idle=true` / `list_idle_workstreams`); marking a workstream *done* is the dossier's job, not a hub status. (3) **Resolution is slug-primary:** `create_workstream` mints the record + provisions a *default* worktree on branch `bt/<slug>` (derived from the unique slug, not the label) + opens a Herdr pane, but persists no path; `basecamp workstream current` derives the slug from the trailing `copilot/<slug>` worktree path segment and looks it up by slug, `basecamp workstream show <slug>`/`attach <slug>` work from anywhere. No `edit_workstream`, no content versioning, no `by-worktree` route. (4) The `src/basecamp/claude/` foundation reads `~/.claude/basecamp.json` (not `~/.pi`); the Logseq resources are `basecamp://logseq/{cockpit,dossiers}`. The sections below are the pre-implementation design and are kept for rationale; treat this note + the code as the spec.
