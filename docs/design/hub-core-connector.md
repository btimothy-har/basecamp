...
4. **Doc-truing** (this commit) — §11.

**Pending — step 4 (internal swarm flatten).** `pi/swarm/agents/daemon/` still holds the agent-side files (rpc client, reporter, delivery, dispatch-retry, event-summaries, run-result, tools, view, widget, `registerAgentSurfaces`). It is fully green and functional; only the directory name is now stale (the daemon connector left for core). Flattening it into `pi/swarm/agents/` (or renaming the dir) is pure internal cosmetics with no cross-domain or behavior impact — deferred pending a call on whether the churn is worth it.
