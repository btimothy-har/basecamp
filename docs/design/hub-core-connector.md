...
6. **Raw-thread reporting → core** (follow-on, post-repivot). `registerThreadReporter` (the `agent_end` hook that ships `getBranch()`) moved from `pi/companion/thread-reporter.ts` to `pi/core/hub/thread-reporter.ts`, and `registerHubConnection` now registers it — making **"connect + report" a single core responsibility**. Rationale: every session feeds the daemon its raw thread regardless of the dashboard, so reporting is core hub functionality, not companion policy (this revises [companion-daemon-broker.md](./companion-daemon-broker.md) §4.1's "companion owns the policy"). Companion keeps only the dashboard integration (snapshot/panes/herdr) and **drops its last `#core/hub` dependency** — it is now a pure downstream consumer of the daemon-derived analysis. The reporter self-gates on subagents (no-op for daemon-spawned agents). Green (704 TS).
