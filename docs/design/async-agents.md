...
- **Exit-path finalization agrees with the report.** The runner writes the sidecar `final` *before* it exits, so the daemon's process-exit reaper (`reap_agent_process`) derives the terminal status from that record — with the same `ok`→`completed` / else→`failed` mapping as `handle_result_report` — rather than blindly marking `failed`. The async `result_report` frame and the process exit are two independent daemon events; deriving both finalizers from the same recorded `final` makes their `set_run_result_if_unset` race benign (whichever wins writes the same status). The `failed` "exited without reporting a result" fallback remains for a runner that dies before recording any final result.
