...
## 15. Execution status (as built)

Shipped as six green (`make lint` + full `make test`) commits on `claude/agent-roles-identities-k7gt1j`, one per §13 tranche. The following deliberate deviations from the design above are the as-built reality — read the `kind` language throughout this record through them:

- **Field kept as `role` (values `agent | worker`), not renamed to `kind`.** The §10-sanctioned low-churn alternative: it delivers the full agent-vs-worker semantics without a cross-language wire/DB/test identifier rename. Every `kind` above is, as built, the `role` column/field carrying `agent | worker`. It is founded on `BASECAMP_USER_FACING` (`=0` stamped on daemon-spawned workers; absent ⇒ `agent`), with a one-shot `PRAGMA user_version` remap of legacy `session→agent` / `agent→worker` rows.
- **Double-live-instance was already guarded.** The daemon already hard-rejects a duplicate live connection (`app.py` `duplicate_node_connection`), so §6.1's guard existed — and correctly, as a hard-fail with a disconnect reaper, not the "warn now" §6.1/§12(d) proposed. It was kept as-is (a warn would permit split-brain); §14's PID-verified stale-reclaim is the remaining refinement.
- **Handoff worktree-reuse keys off the worktree label, not `is_workstream_agent`.** `shouldReuseActiveWorktreeForHandoff(activeWorktree)` reuses when the active worktree is `copilot/`-prefixed (a workstream worktree) — the §6 behavior-preserving alternative — which also avoids the `tasks → workstreams` cross-domain edge that reading membership would have introduced.
- **`repo` / `worktree_label` are preserve-on-conflict** in `upsert_agent`, and the runner proxy forwards them when re-registering a worker — so a worker's facets survive a lightweight result-report re-register.
- **Expiry is `last_seen_at`-based, no `expires_at` column.** `list_agents` drops rows that have no live connection and whose `last_seen_at` is older than the fixed 24h `AGENT_TTL_HOURS` constant (a live connection always exempts a row); the row is retained. `run_kind` and `product_role` are removed via a guarded `DROP COLUMN` migration; the mutative/read-only tool split collapses to one write-capable `AGENT_TOOLS` set.
- **One protocol bump (v21)** covers every register / dispatch / list-agents frame change across the feature.
