...
OMP `/review` keeps OMP's native scope selection, diff handling, and reviewer fan-out. Basecamp adds a final `review_findings` presentation step: validated P0–P3 findings open in a keyboard navigator, optional comments are stored with their findings in a session `artifact://` payload, and the agent reads that artifact before continuing. In headless modes the same payload is saved with interactive feedback marked unavailable.
