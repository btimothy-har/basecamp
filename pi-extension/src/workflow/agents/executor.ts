...
import type { TaskProgressSnapshot, TaskProgressStatus, TaskProgressTask } from "../tasks/render.ts";
import {
	type AgentConfig,
	getAgentRunKind,
...
export interface AgentSkillContentDeps {
...
export interface PiAgentSkillDeps extends AgentSkillContentDeps {
	resolveSkills: (skillNames: string[], cwd: string, deps: AgentSkillContentDeps) => SkillResolutionResult;
	buildSkillInjection: (skills: ResolvedAgentSkill[], deps: AgentSkillContentDeps) => string;
}
...
export function buildPiArgs(
	...
	if (agent?.skills?.length) {
		const { resolved } = deps.resolveSkills(agent.skills, opts.cwd, deps);
		if (resolved.length > 0) {
			skillInjection = deps.buildSkillInjection(resolved, deps);
		}
