...
export function registerPlanCommands(pi: ExtensionAPI, tasksAccess: TasksAccess, plan: PlanAccess): void {
	pi.registerCommand("plan", {
		description: "Explore a topic and formalise an implementation plan",
		handler: async (args, ctx) => {
			...
			pi.sendUserMessage(
				`I want to explore and plan: ${topic}\n\nInvoke the \`planning\` skill and follow its full process. Do not skip phases — explore the problem space first, discuss approach with me, then formalise via \`plan()\`. Do not prototype or edit code before the plan is approved.`,
			);
		},
	});
