...
from pi_observer.llm import agents
from pi_observer.llm.agents import ExtractionResult, SummaryResult
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    Patches the pi_observer.settings singleton to use a temp config file,
    preventing tests from reading or overwriting real config.
    """
    obs = tmp_path / "observer"
    obs.mkdir()
    monkeypatch.setattr(constants, "OBSERVER_DIR", obs)
    ...
    monkeypatch.setattr("pi_observer.settings.settings", test_settings)
    monkeypatch.setattr("pi_observer.llm.agents.settings", test_settings)
    monkeypatch.setattr("pi_observer.cli.main.settings", test_settings)
...
def db(tmp_path, monkeypatch) -> Database:
    ...
    monkeypatch.setattr(constants, "OBSERVER_DIR", tmp_path)
    ...
    monkeypatch.setattr("pi_observer.services.db.DB_URL", db_url)
    monkeypatch.setattr("pi_observer.services.db.OBSERVER_DIR", tmp_path)
