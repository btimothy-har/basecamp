...
from pi_observer.data.enums import RawEventStatus, WorkItemStage, WorkItemType
from pi_observer.data.raw_event import RawEvent
from pi_observer.data.schemas import ProjectSchema, RawEventSchema, TranscriptEventSchema, TranscriptSchema
from pi_observer.data.work_item import WorkItem
from pi_observer.llm import agents
from pi_observer.pipeline.grouping import EventGrouper, classify_events
from pi_observer.pipeline.refinement import EventRefiner, WorkItemRefiner
...
class TestRefineBatch:
    ...
    def test_thinking_llm_failure_uses_fallback(self, db, tmp_path):
        ...
        EventGrouper.group_pending(db, transcript_id)
        with patch.object(agents.thinking_summarizer, "run", new=AsyncMock(side_effect=RuntimeError("LLM failed"))):
            EventRefiner.refine_pending(db)
