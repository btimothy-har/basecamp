...
function triggerIngest(pi: ExtensionAPI, ctx: ExtensionContext, processFlag: boolean): void {
	...
	const cwd = process.env.BASECAMP_WORKTREE_DIR?.trim() || path.resolve(ctx.cwd);
	const repoName = process.env.BASECAMP_REPO?.trim() || null;
	...
	const observerCmd = processFlag ? "pi-observer ingest --process" : "pi-observer ingest";
	...
	pi.exec("bash", ["-c", shellCmd], { timeout: 300_000 }).catch(() => {
		if (!notifiedFailure) {
			...
			ctx.ui.notify("pi-observer ingest failed — check ~/.pi/observer/observer.log", "warning");
		}
	});
}
