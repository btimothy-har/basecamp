...
Project-aware Pi extension for AI coding agents. Basecamp configures project context, manages isolated git worktrees, and provides workflow guardrails for coding sessions.
...
Basecamp no longer ships an automatic session-history or recall service. The extension stays focused on project context, prompt control, workflow commands, and worktree safety. Existing local session-history stores are not read by Basecamp and can be removed manually if they are no longer needed.
