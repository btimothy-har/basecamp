...
The shared installer also registers Basecamp's standalone workspace policy extension in the active directory reported by `omp config path`, so plain `omp` receives it too. In an interactive canonical checkout it warns once; every turn tells the agent to use a worktree for implementation; and known structured `write`, `edit`, `ast_edit`, and applying `lsp` mutations into the canonical checkout are blocked. This is an accidental-mutation guard, not a sandbox: shell, eval, task, custom, and raw LSP execution remain native OMP behavior.
