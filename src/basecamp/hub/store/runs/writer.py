...
from .._sqlite import dump_json_column
        ...
        *,
        ...
        with self._writing() as connection:
            ...
            connection.execute(
                ...
                VALUES (?, ?, 'running', ?, ?, ?, ?, ?)
                """,
                (run_id, agent_id, dispatcher_id, dump_json_column(spec), report_token_hash, now, now),
            )
            connection.execute(
    ...
    def append_run_event(self, *, run_id: str, kind: str, payload: dict[str, Any]) -> int:
        ...
        with self._writing(immediate=True) as connection:
            ...
            connection.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (run_id, int(next_seq), kind, dump_json_column(payload), self._now()),
            )
            return int(next_seq)
