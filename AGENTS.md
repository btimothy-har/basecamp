...
Every dispatched agent runs in its **own transient git worktree** with the uniform toolset (including `write`/`edit`) — there is no read-only dispatch posture (issue #310, Phase 1). The workspace bases on the parent worktree's HEAD, or on a **snapshot commit** of its dirty state built via a throwaway `GIT_INDEX_FILE` (tracked + untracked captured; the parent's tree/index/HEAD untouched), so agents always see the parent's WIP. Branches are **per agent** (`agent/<handle>`), worktrees per run (`agent-<runToken>/<name>`): a retask continues the agent's outstanding branch (its memory and tree never contradict), a branch already merged into the parent/default branch is deleted eagerly at provision for a fresh base, and a *fresh* dispatch that finds a pre-existing branch fails rather than silently adopting foreign work. Ask answerers get a **detached** workspace at the ask target's branch tip (else parent HEAD/snapshot) and never mint a branch. Setup hooks (`environments.setup`) run blocking-but-nonfatal on dispatch workspaces; asks skip them.

**Commits are the only durable output of a run** (this replaced "never force-remove post-execution work"). At run end the daemon force-removes the workspace — in both the normal reap and the crash-restart reconcile, driven by the dispatch spec's `owned_branch`/`branch_base`/`branch_created` — deleting the branch only when this run minted it and it gained no commits past its recorded base OID. The session-start sweep is the backstop when the daemon never restarted: it reclaims agent worktrees whose branch is integrated or still snapshot-only, and never touches unintegrated real work. The primary integrates by `git merge agent/<handle>`. Dispatched agents get a teardown-aware dirty reminder (commit deliverables; scratch vanishes by design); primary sessions keep the advisory commit reminder. The worktree is the isolation boundary, enforced by the workspace guard's `allowed_dirs` rule; `bash` is deliberately retained and is **not** a mutation sandbox — the workspace, not the toolset, is the wall. Independently, the workspace `tool_call` guard hard-blocks structured `write`/`edit` to the protected main checkout even when a subagent has no active worktree.
