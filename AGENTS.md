...
The OMP migration target is a separate Bun package rooted at `omp/`; its manifest lists the ordinary `omp/extension.ts` entrypoint and the independently loadable `omp/workspace/workspace.ts` policy entrypoint. The shared installer also registers only the workspace entrypoint for ambient plain-OMP discovery. Distinct basenames keep OMP's capability/disable keys unambiguous. OMP executable modules use only OMP APIs and never import legacy Pi code.
...
A fresh interactive `bomp` launch whose effective cwd is inside the canonical checkout is supervised in an automatic scratch. The launcher captures a coherent Git snapshot of `HEAD`, index semantics, tracked content, and nonignored untracked content without changing the source checkout; creates a detached worktree at the pinned `HEAD` through public `omp worktree add`; restores the snapshot there; and starts OMP at the corresponding nested cwd. Explicit session sources and OMP's settings-driven auto-resume, management/headless modes, non-TTY launches, linked-worktree launches, and explicit `--direct` remain direct.

Automatic scratch ownership remains entirely in the Python supervisor; no launcher-to-extension state record describes the workspace. OMP's transcript cwd is the sole session/worktree affinity record. On final exit, Basecamp removes the initial scratch only when it is still detached and either semantically matches the launch snapshot or has been reset clean to the same pinned `HEAD` by native `/wt` cleanup. Branch-attached worktrees, detached commits, changed content, nondefault index state, and uncertain Git state are retained. OMP's native `/wt <branch>` carries the current changes and session into a durable branch worktree. Private snapshot refs and temporary indexes are released after supervision; Basecamp adds no resume adapter or persistent workspace registry.
...
The standalone workspace extension uses OMP's native Git discovery on startup, session switch, and prompt. Canonical interactive TUI sessions warn once per entry. Every Git-backed prompt receives the same reminder: canonical checkouts are for inspection and planning, detached worktrees are writable but temporary, and branch-backed worktrees are durable. This policy is advisory; the extension registers no mutation guard or parallel workspace-state adapter.
