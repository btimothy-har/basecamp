...
Every **dispatched** agent (scouts, workers, `ask` answerers, the six `/code-review` reviewers) is **read-only**: `getAgentToolAllowlist()` (`pi/core/swarm/agents/types.ts`) yields one uniform toolset without `write`/`edit`, and each agent is launched with `--read-only`. The **primary session is the sole mutator** — subagents investigate, review, and report; the main agent applies the changes (the `worker` agent returns a change proposal rather than editing). `bash` is retained (scouts need `git log`/`gh`, reviewers need `git diff`) and is deliberately **not** a mutation sandbox — a bash write can still reach the filesystem, so read-only is defense-in-depth, not a wall. True bash containment (and safely re-enabling *mutative* agents, writing results back as a host-applied diff) is the container-isolation direction recorded in `docs/design/agent-isolation.md`. Independently, the workspace `tool_call` guard (`pi/core/project/workspace/guards.ts`) hard-blocks structured `write`/`edit` to the protected main checkout even when a subagent has no active worktree.
