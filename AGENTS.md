...
- **Ownership rule**: every layer answers exactly one question. `environment.md` = what is true here; `buildEnvBlock` = what is true right now; **tool description = how to call it**; `modes/*` = **what are we doing**; `styles/*` = **how it is achieved** (who you are / how you work), `voice.md` = **how output is shaped**, `craft.md` = **how code is written** — the role style is selectable, voice reaches every primary session, craft is unconditional; skills = how to do it well in depth; project context = repo gotchas.
