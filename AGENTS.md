...
The Terminal-Bench adapter exposes two profiles selected by `run.py --profile` / `EVAL_PROFILE`. **`basecamp-pi-single`** archives `package.json`, `package-lock.json`, and `pi/` from an exact Git commit, pins `BASECAMP_AGENT_DEPTH=1` / `MAX_DEPTH=1` to select the no-daemon, no-dispatch surface, and requires three explicit launch signals for structured mutation (`--unsafe-edit`, `--unsafe-edit-sandboxed`, `BASECAMP_EXTERNAL_SANDBOX=1`; read-only still wins). An optional `models.json` is digest-verified and installed `0600` with environment-backed credentials; no auth state or secret enters the archive. `make eval*` pins run inputs and requires a clean commit for executable runs; both profiles produce local Harbor scores and Pi logs only — no ATIF and no leaderboard claim.

**`basecamp-pi-swarm`** measures the dispatch surface: the session is top-level (`DEPTH=0`, `MAX_DEPTH=1`, so children cannot re-dispatch), the archive additionally ships `pyproject.toml`/`src`/`uv.lock`, and the container gets a locked venv plus a preflighted, left-running hub daemon. The task workdir is git-initialised on demand (idempotent; files over 50M stay out via `.git/info/exclude`; pre-existing task repos untouched) so the production deliverable posture — worktree per agent, branch from clean HEAD, merge integration — applies unchanged; dispatched agents ride the trial model (persona aliases degrade to inherit without `config.json`, with the fallback surfaced in the dispatch result) and vary only by thinking level. Swarm state (child transcripts, run sidecars, `daemon.db`) is harvested into `/logs/agent/swarm/` after every run.

The archive carries no `config.json`, so the `fast` alias cannot resolve in a trial: the **bash reviewer**, which depends on it, is switched off rather than silently degraded, because its no-UI failsafe is fail-closed and would hard-block every gated command instead of judging it. The **continuation guard** does not depend on the alias — its judge rides the active session model, which the installed `models.json` provides — so it is live in trials. `BASECAMP_BASH_REVIEWER=off` is honoured only alongside `BASECAMP_EXTERNAL_SANDBOX=1` **and** the `--unsafe-edit-sandboxed` launch flag — env and argv are independent channels, so the environment alone can never strip the gate from a real session (`loadDotenv` also refuses `BASECAMP_*` keys). Dispatched children inherit the trio only as a whole: the spawn env strips the pair unless the parent's own argv carries `--unsafe-edit-sandboxed`, in which case the pair and the flag propagate together — so the chain is always rooted in a human-issued launch flag. Trial metadata records both states.
