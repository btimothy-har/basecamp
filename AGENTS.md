...
The archive carries no `config.json`, so the shared `fast` alias cannot resolve in a trial and both its consumers are excluded rather than silently degraded: the **bash reviewer** is switched off, because its no-UI failsafe is fail-closed and would hard-block every gated command instead of judging it, and the **continuation guard** is inert, because its posture is fail-open. `BASECAMP_BASH_REVIEWER=off` is honoured only alongside `BASECAMP_EXTERNAL_SANDBOX=1` **and** the `--unsafe-edit-sandboxed` launch flag — env and argv are independent channels, so the environment alone can never strip the gate from a real session (`loadDotenv` also refuses `BASECAMP_*` keys, and agent spawn env strips the pair). Trial metadata records both exclusions.
