---
name: marimo
description: "Guidance for work involving marimo reactive notebooks. Keywords: marimo, reactive notebook, data app, mo.ui."
---

# Marimo Reactive Notebooks

Extends the `skill://python-development` skill. Marimo is a reactive Python notebook—stored as pure `.py` files, git-friendly, executable as scripts, and deployable as apps.
...
def expensive_computation(x: int) -> int:
    # Result is cached in memory based on input
    return x ** 2
...
For expensive notebooks, configure lazy runtime so edited cells are marked stale instead of auto-running. Set it in notebook settings, or in `pyproject.toml`:
