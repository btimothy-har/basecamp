...
Consistent formatting improves readability and reduces cognitive load. This document describes one SQLFluff-aligned house style (trailing commas, numeric GROUP BY, BigQuery-oriented patterns). Apply it in projects configured for it; in other projects, match the existing lint config and conventions instead.

## Trailing Commas

**In this style, use trailing commas** after the last item in a SELECT clause (before FROM). This is valid in BigQuery and some other modern dialects, but not universal — do not introduce it in codebases or dialects that do not already use it.
...
-- Group keys come first in the SELECT so numeric positions line up
SELECT
  loc.location_id,
  DATE_TRUNC(sp.payment_date, MONTH) AS payment_month_start,
...
In this style, prefer **numeric references** (`GROUP BY 1, 2`) for DRY — valid in BigQuery and PostgreSQL, but not every dialect; where numeric references are unsupported or the project names columns, group by named columns instead. Rules:
...
`GROUP BY ALL` is BigQuery-specific (also supported by Snowflake and some others). Use it sparingly, primarily for:
