...
{% if is_incremental() %}
    ...
    WHERE event_date >= DATE_SUB(
        COALESCE((SELECT MAX(event_date) FROM {{ this }}), DATE '1900-01-01'),
        INTERVAL 3 DAY
    )
{% endif %}
```
...
Granularity may be `'day'`, `'month'`, or `'year'`.
...
{% if is_incremental() %}
    ...
    WHERE event_timestamp >= TIMESTAMP_SUB(
        COALESCE((SELECT MAX(event_timestamp) FROM {{ this }}), TIMESTAMP '1900-01-01'),
        INTERVAL 7 DAY
    )
{% endif %}
```
...
{% if is_incremental() %}
    WHERE order_date >= DATE_SUB(
        COALESCE((SELECT MAX(order_date) FROM {{ this }}), DATE '2020-01-01'),
        INTERVAL 3 DAY
    )
{% else %}
