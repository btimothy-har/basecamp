...
COPY . .
# Runtime must not resync (or reinstall dev deps): --no-sync skips the
# environment check uv run performs by default.
...
uv add ./packages/pkg-a         # Add workspace dependency by path
```
