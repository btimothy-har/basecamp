"""Tests for the doctrine managed-block install in basecamp.setup."""
...
from basecamp import setup
from basecamp.setup import _DOCTRINE_BEGIN, _DOCTRINE_END, _install_doctrine, _upsert_managed_block
...
def _patch_source(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, body: str) -> None:
    ...
    monkeypatch.setattr(setup, "shipped_prompts_dir", lambda: prompts)
...
def test_install_doctrine_missing_source(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setattr(setup, "shipped_prompts_dir", lambda: tmp_path / "nope")
