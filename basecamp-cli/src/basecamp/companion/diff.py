...
def list_changed_files(git: GitRunner, base_commit: str) -> list[FileStatus]:
    """List changed (committed/uncommitted/untracked) files vs base commit."""

    entries_by_path: dict[str, FileStatus] = {}

    code, output = git(["diff", "--name-status", "-M", base_commit, "--"])
    if code == 0:
...
def collect_changes(git: GitRunner) -> tuple[str | None, list[FileStatus]]:
    """Collect base commit and changed file list for the current checkout."""
...
def file_diff_lines(
    ...
    """Return placeholder message plus full-file diff lines for one file."""

    current_path = cwd / file.path
    ...
    base_text = read_base_content(git, base_commit, file.path)
    ...
    base_line_count = _line_count(base_text)
    current_line_count = _line_count(current_text)
    max_line_count = max(base_line_count, current_line_count)
