...
For BigQuery, save SQL to a `.sql` file, then run it with the `bq` CLI. Always dry-run first: a dry run validates the query and reports the bytes it will scan, so you can catch an expensive full-table scan before paying for it.

```bash
# Validate + estimate scanned bytes (no execution, no cost)
bq query --dry_run --use_legacy_sql=false < query.sql

# Execute once the estimate looks reasonable; cap returned rows
bq query --use_legacy_sql=false --max_rows=1000 < query.sql
```

Filter on partition columns to keep scanned bytes (and cost) down. For large result sets, write to a destination table with `--destination_table` rather than returning rows to the terminal.
