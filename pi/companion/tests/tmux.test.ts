	...
	describe("buildCompanionCommand", () => {
		it("quotes snapshot path and cwd", () => {
			assert.equal(
				...
				"basecamp companion tui --snapshot '/tmp/with space/snapshot.json' --cwd '/tmp/worktree cwd'",
			);
		});
		...
		it("escapes single quotes", () => {
			assert.equal(
				...
				"basecamp companion tui --snapshot '/tmp/it'\\''s-snapshot.json' --cwd '/tmp/it'\\''s-cwd'",
			);
		});
		...
		it("appends quoted scratch dir when provided", () => {
			assert.equal(
				...
				"basecamp companion tui --snapshot '/tmp/snap.json' --cwd '/tmp/cwd' --scratch '/tmp/pi/basecamp'",
			);
		});
	});
