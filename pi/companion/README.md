...
- **Analysis thread reporter**: on each `agent_end` (top-level sessions only), ships the raw session thread (`getBranch()`, split into per-entry nodes) to the daemon for the daemon-side analyzer, via `#swarm`'s `reportThread` transport (`thread-reporter.ts`)
...
- **swarm** (`#swarm/index.ts`): `reportThread` — ships the raw session thread to the daemon for the analyzer
