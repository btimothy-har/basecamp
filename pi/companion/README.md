...
Basecamp companion — session snapshot hooks, companion panes, and Herdr metadata.
...
- **tasks** (`#tasks/index.ts`): `getTasksAccess` + `TaskStatus` — live task-state observation for the snapshot

## Observation pattern

Companion reads task state from the tasks context via `getTasksAccess()` (`#tasks/index.ts`). Until tasks registers its implementation, `getTasksAccess()` returns null and the snapshot omits task progress.
