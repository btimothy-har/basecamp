...
import { type CardState, listItems, ReactionStore, reduceCard } from "#code-review/annotate/model.ts";
...
describe("ReactionStore", () => {
	it("trims comments and clears the entry when the text is blank", () => {
		const store = new ReactionStore(3);
	...
	it("aligns reactions to findings by index and nulls the uncommented", () => {
		const store = new ReactionStore(4);
		store.set(0, "intentional");
		store.set(2, "question about this");

		assert.deepEqual(store.toReactions(), ["intentional", null, "question about this", null]);
	});
});
