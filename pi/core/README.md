...
- **Hub connector** (`hub/`): core's adapter for the hub daemon — the WebSocket transport + register/handshake, ensure-daemon (spawn/health/version), node-identity derivation, the connection-status footer, and the raw-thread `reportThread` transport, plus the shared **wire protocol** (`hub/protocol/`: the TS codec + frame types, the JSON fixtures, and `PROTOCOL.md`, kept in lockstep with the Python `basecamp.hub`). `registerCore` opens the connection at `session_start` for top-level sessions and daemon-spawned agents, exposing `awaitDaemonConnection`/`onDaemonConnect`; `swarm` (agent tools/reporting/widget) and `companion` (thread reporting) consume it via `#core/hub`.
