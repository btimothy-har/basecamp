...
- **Hub connector** (`hub/`): core's adapter for the hub daemon — the WebSocket transport + register/handshake, ensure-daemon (spawn/health/version), node-identity derivation, the connection-status footer, and the **raw-thread reporter** (`thread-reporter.ts` — ships `getBranch()` at `agent_end`) with its `reportThread` transport, plus the shared **wire protocol** (`hub/protocol/`: the TS codec + frame types, the JSON fixtures, and `PROTOCOL.md`, kept in lockstep with the Python `basecamp.hub`). `registerCore` opens the connection at `session_start` for top-level sessions and daemon-spawned agents **and registers the reporter** — "connect + report" is one core responsibility — exposing `awaitDaemonConnection`/`onDaemonConnect`. `swarm` (agent tools/reporting/widget) consumes the connection via `#core/hub`; `companion` no longer produces analysis data — the daemon derives it from the reported thread and companion is a downstream consumer.
