...
describe("list_agents", () => {
	...
	it("sends request, waits on request id, and formats response rows", async () => {
	...
	it("rejects when the daemon connection closes before a response", async () => {
		const connection = new MockConnection();
	...
	it("requires agents skill invocation", async () => {
		let connected = false;
		const { pi, tools } = createMockPi();
		registerDaemonTools(
			pi,
