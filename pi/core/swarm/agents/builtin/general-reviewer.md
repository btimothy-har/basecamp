...
2. **Trace logic and data flow** — Follow inputs, state changes, control paths, side effects, error paths, async ordering, and resource lifecycles from entry to outcome
3. **Verify reachability and impact** — Confirm each issue is real, reachable, and not already mitigated; avoid speculation and explain the observable failure mode
4. **Report findings only** — Do not make changes or write fixes — provide your general review findings
