...
- **Agent catalog** (`discovery`, `catalog`, `builtin/*.md`) — the basecamp-owned builtin agents (`scout`, the review specialists), published to the core catalog registry (`#core/catalog`) as the `agents` capability type. Core can't enumerate these itself (they aren't pi-native), so the primitive supplies the provider; the definitions are its standard library.
