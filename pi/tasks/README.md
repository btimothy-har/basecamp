...
The judge rides the **active session model**, with auth from the model registry — no alias, no separate configuration, and no off switch beyond the read-only flag and the mechanical preconditions. The bash reviewer deliberately keeps the `fast` alias instead, and the asymmetry is load-bearing: the reviewer runs on every bash call and is fail-*closed*, so it needs a cheap dedicated model, while the guard runs once per stop and is fail-open, so it can afford the session model. Do not unify them.
