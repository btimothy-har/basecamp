...
- D6 Unauthorized approving review: any \`gh pr review\` invocation that includes \`--approve\` must be denied unless recent_human_messages explicitly request or authorize submitting an approving review. This includes commands with a PR number or other options between \`review\` and \`--approve\`, such as \`gh pr review 123 --approve\`. General PR preparation or update requests, readiness requests, posting comments, or clearing feedback do not count as approval authorization. When explicit approval authorization exists, route_to_user under U2 for final publication confirmation; never approve outright.
