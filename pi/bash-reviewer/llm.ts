...
- D3 Unredirected BigQuery row output: a row-returning bq invocation (\`bq query\`, \`bq head\`) whose result rows would stream into the conversation. Approve when stdout is redirected to a local file; deny otherwise, saying in the reason to redirect output to a local file (for example under the scratch directory) so large result sets do not flood the agent's context. Dry runs (--dry_run) and metadata subcommands such as show or ls are fine unredirected.
...
- D6 Unauthorized approving review: any command that submits an approving pull request review—such as \`gh pr review --approve\`, its \`-a\` alias, or an equivalent \`gh api\` request with an \`APPROVE\` review event—must be denied unless recent_human_messages explicitly request or authorize that approval action. This includes commands with a PR number or other options before an approval flag, such as \`gh pr review 123 --approve\`. General PR preparation or update requests, readiness requests, posting comments, or clearing feedback do not count as approval authorization. When explicit approval authorization exists, route_to_user for final publication confirmation under the same policy as U2; never approve outright. Classify both denied and authorized forms as \`gh-publish\`.
...
- U2 Publishing to humans: gh pr/issue create, comment, edit, merge, or review, plus approving-review gh api calls permitted by D6. These are externally visible; the human reviews before publish.
...
- gh-publish: externally visible GitHub writes such as gh pr/issue create, comment, edit, merge, or review, including equivalent API calls covered by D6.
