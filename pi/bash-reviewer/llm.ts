...
R5 Writes to the protected checkout (the main repo working tree, not an active worktree) are suspicious defense-in-depth signals; the edit layer already guards this, but treat such bash writes with caution. This rule does NOT apply to commands running inside an active worktree — those are the intended edit target. Local file edits via bash (sed, tee, echo redirection, perl -i, etc.) inside the working tree or active worktree are normal development operations; approve with risk "local" unless the edit targets files outside the project or would exfiltrate secrets.
