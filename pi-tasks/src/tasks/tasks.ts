 ...
 *   - complete_task: mark a task as done, optionally stopping work
...
export type {
	...
	TasksAccess,
	TasksState,
} from "pi-core/platform/tasks-access.ts";

// Import the types we use locally.
import type { GoalCycle, Task, TaskStatus, TasksAccess, TasksState } from "pi-core/platform/tasks-access.ts";
...
interface CompleteTaskResultDetails {
	task: number;
	label: string;
	stop_work: boolean;
...
function buildCompleteTaskResultText(state: TasksState, index: number, task: Task, stopMessage: string | null): string {
...
export function registerTasks(pi: ExtensionAPI): TasksAccess {
	...
	pi.registerTool({
		...
		async execute(_id, params) {
			...
			persistState();
			return {
				content: [{ type: "text", text: buildCompleteTaskResultText(state, params.task, target, stopMessage) }],
				details: {
					task: params.task,
					label: target.label,
					stop_work: stopWork,
