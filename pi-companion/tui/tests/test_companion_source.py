...
def _write_analysis(path: Path, decisions: list[str]) -> None:
    path.write_text(
        json.dumps({"version": 1, "sessionId": "s", "updatedAt": "t", "decisions": decisions}),
        encoding="utf-8",
    )
...
def test_poll_merges_goals_and_analysis(tmp_path: Path) -> None:
    ...
    assert model.analysis.decisions == ["dec1"]
...
def test_poll_detects_analysis_change_keeping_goals(tmp_path: Path) -> None:
    ...
    _write_analysis(analysis, ["dec2"])
    model = source.poll()
    assert model is not None
    assert model.analysis is not None
    assert model.analysis.decisions == ["dec2"]
    assert len(model.goals) == 1
