...
function buildSteerContent(state: TasksState): string | null {
...
export function registerTasks(pi: ExtensionAPI): void {
	...
	function persistState(): void {
		pi.appendEntry("tasks-state", state);
	}
	...
	pi.on("session_start", async (_event, sessionCtx) => {
		...
		const tasksEntry = entries
			.filter((e) => e.type === "custom" && (e as { customType?: string }).customType === "tasks-state")
			.pop() as { data?: TasksState } | undefined;

		if (tasksEntry?.data) {
			if (tasksEntry.data.goal) state.goal = tasksEntry.data.goal;
			if (tasksEntry.data.tasks) state.tasks = tasksEntry.data.tasks;
		}
